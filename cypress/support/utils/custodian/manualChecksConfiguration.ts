...
const addManualChecksForUsersConfigurationManualChecks = () => {
    ...
    cy.get(dataCy("form-modal")).within(el => {
        console.log(el.find("button").map(item => item));
      });
};
...
const hasUnCheckedOnOrganisationConfigurationManualChecks = () => {
    ...
    cy.contains("p", "Is the Organisation aligned with the SDE network?").should("exist");
    cy.get('[data-cy="action-menu"]')
      .find('button[type="button"]')
      .should("exist");
    cy.get("#4").should("exist").uncheck();
    cy.contains("p", "Are we confident costs would be met and profits realised for future projects?").should("exist");
    cy.get('[data-cy="action-menu"]')
      .find('button[type="button"]')
      .should("exist");
};
